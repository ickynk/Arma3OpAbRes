...
//   Currently testing with ALL AI ENABLED (no components disabled).
//   If helicopters deviate from recorded path, uncomment AI disabling code and
//   gradually disable components: TARGET/AUTOTARGET → MOVE → PATH → FSM
...
for "_i" from 0 to ((count _assaultHelis) - 1) do {
  ...
  // TEST CONFIG: NO AI components disabled - testing if BIS_fnc_unitPlay works with full AI
  // If helicopters deviate from recorded path, uncomment the AI disabling block below
  ...
  [_veh] remoteExec [
    {
      ...
      if (!isNull _p) then {
        // Uncomment components that interfere with playback:
